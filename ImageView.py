...
class ImageView():
    def __init__(self,master,image_list):
        ...
        self.crop = CTkButton(self.toolbar,width=50,height=40,text="",image=self.img0,corner_radius=0,fg_color="#760526",hover_color="#760526",command=self.crop_selection)
        ...
        self.canvas = Canvas(self.image_frame,width=760,height=390,background="#5A011B",highlightthickness=0)
    ...
    def crop_selection(self):
        ...
        self.border = self.canvas.create_rectangle(self.x1,self.y1,self.x2,self.y2,width=3,outline='white')
        self.canvas.bind('<Motion>',lambda Event: self.get_selection(Event))
        # print(coordinates)

    def get_selection(self,Event):
        if Event.x >= self.x1 and Event.x <= self.x2 and Event.y == self.y1:
        ...
        elif Event.x >= self.x1 and Event.x <= self.x2 and Event.y == self.y2:
            self.canvas.configure(cursor='sb_v_double_arrow')
            self.canvas.unbind('<Button-1>')
        ...
        elif Event.y >= self.y1 and Event.y <= self.y2 and Event.x ==self.x1:
        ...
        elif Event.y >= self.y1 and Event.y <= self.y2 and Event.x ==self.x2:
            self.canvas.configure(cursor='sb_h_double_arrow')
            self.canvas.unbind('<Button-1>')
        ...
        if x_or_y == 'x':
            if a_or_b == 'a':
                if Event.y >= self.y1 and Event.y < self.y2:
                    self.canvas.coords(self.border,self.x1,Event.y,self.x2,self.y2)
            elif a_or_b == 'b':
                if Event.y <= self.y2 and Event.y > self.y1:
                    self.canvas.coords(self.border,self.x1,self.y1,self.x2,Event.y)
        
        elif x_or_y == 'y':
            if a_or_b == 'a':
                if Event.x >= self.x1 and Event.x < self.x2:
                    self.canvas.coords(self.border,Event.x,self.y1,self.x2,self.y2)
            elif a_or_b == 'b':
                if Event.x <= self.x2 and Event.x > self.x1:
                    self.canvas.coords(self.border,self.x1,self.y1,Event.x,self.y2)
